...
from luxurylp.logger import logger, Logger, LoggerLevel

from luxurylp import const
from luxurylp.commands import commands_bp
from luxurylp.error_handlers import error_handlers_bp
from luxurylp.database import Database, DatabaseError
from luxurylp.utils import check_ping
...
parser = argparse.ArgumentParser(
    description='LP модуль позволяет работать приемнику сигналов «LuxuryDuty» работать в любых чатах.\n'
...
parser.add_argument(
    ...
    action="store_const",
    const=True,
    help='Использовать папку AppData/LuxuryDuty (Windows).\n'
         'При использовании этой настройки AppData/LuxuryDuty и config_path складываются'
)
...
def run_lp():
    ...
    logger.warning(
        ...
        f" -> Использовать папку AppData/LuxuryDuty  -> {const.USE_APP_DATA}\n"
    ...
    try:
        ...
        from luxurylp.validators import (
            alias,
            role_play_command,
            self_prefix,
            duty_prefix,
